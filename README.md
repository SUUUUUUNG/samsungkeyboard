...
## 6. 다음 단계 (막힌 지점을 뚫으려면)
목표는 **SW 출력값(= 기준값)의 재현**입니다. 기준값 "실제(mm)"는 SW의 P1–P15 거리와 0.05mm 이내로 같으므로 SW 출력이고, 캘리퍼 실측값은 없습니다(확인됨). 따라서 SW 자체의 정확도는 알 수 없고, P15를 0.5mm 수준으로 재현하려면 SW가 P15를 잡을 때 본 것과 같은 입력이 필요합니다. 우선순위 순:
...
3. **Size Korea 393 항목의 공식 정의 확인**: P15가 주름점인지 관절점인지 문서로 확정되면 1번에서 어떤 출력물이 결정적인지 알 수 있습니다.
